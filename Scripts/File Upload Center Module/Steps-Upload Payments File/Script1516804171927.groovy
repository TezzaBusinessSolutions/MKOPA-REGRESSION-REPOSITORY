...
if (blankComment == true) {
    WebUI.verifyElementVisible(findTestObject('File Upload Center Module/Upload Payment File (MPESA)/errorMsg_Please Enter The Notes'), 
        FailureHandling.STOP_ON_FAILURE)
}

if (blankTag == true) {
    WebUI.verifyElementVisible(findTestObject('File Upload Center Module/Upload Payment File (MPESA)/errorMsg_Please Enter The Tag'), 
        FailureHandling.STOP_ON_FAILURE)
}
